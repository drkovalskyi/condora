#!/usr/bin/env bash
# setup-dev-vm.sh — Provision a fresh AlmaLinux 9 VM for Condora development
#
...
#
# Usage:
#   sudo CONDORA_REPO_URL=https://github.com/your-org/condora.git bash scripts/setup-dev-vm.sh
#
# If the repo is already cloned at /mnt/shared/work/condora, CONDORA_REPO_URL is optional.
#
# After completion:
#   su - agent
#   cd /mnt/shared/work/condora
...
# ── Configuration ─────────────────────────────────────────────
DEV_USER="agent"
REPO_DIR="/mnt/shared/work/condora"
VENV_DIR="$REPO_DIR/.venv"
PG_DB="condora"
PG_TEST_DB="condoratest"
PG_USER="condora"
PG_PASS="condoradev"
REPO_URL="${CONDORA_REPO_URL:-}"   # set this or export CONDORA_REPO_URL before running
SITECONF_DIR="/opt/cms/siteconf"
CONDOR_CONFIG="/etc/condor/config.d/50-condora-dev.conf"
...
# Condora dev user — limited sudo for service management and package install
...
# Condora development personal condor configuration
...
if [[ ! -d "$REPO_DIR" ]]; then
    if [[ -n "$REPO_URL" ]]; then
        ...
        echo -e "  ${YELLOW}WARN${NC}  Repo not cloned — set CONDORA_REPO_URL and re-run, or clone manually:"
...
MARKER="# Condora dev environment"
...
# Condora Local Environment — $HOSTNAME_VAL
