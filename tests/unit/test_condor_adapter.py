...
def adapter(mock_htcondor2):
    ...
    # Populate the cached schedd so tests can mock methods on it
    mock_schedd = mock_htcondor2.Schedd.return_value
    adapter._schedds[adapter._default_schedd_name] = mock_schedd
    adapter._default_schedd = mock_schedd
    return adapter
...
async def test_submit_dag(adapter):
    ...
    adapter._default_schedd.submit.return_value = mock_result
...
async def test_query_job_found(adapter):
    ...
    adapter._default_schedd.query.return_value = [mock_ad]
...
async def test_query_job_not_found(adapter):
    adapter._default_schedd.query.return_value = []
...
async def test_check_job_completed_in_history(adapter):
    adapter._default_schedd.query.return_value = []  # not in queue

    mock_ad = MagicMock()
    mock_ad.__getitem__ = lambda self, k: 4  # Completed
    adapter._default_schedd.history.return_value = [mock_ad]
...
async def test_check_job_completed_gone_from_history(adapter):
    adapter._default_schedd.query.return_value = []
    adapter._default_schedd.history.return_value = []  # not found anywhere
...
async def test_remove_job(adapter):
    ...
    adapter._default_schedd.act.assert_called_once()
...
async def test_ping_schedd_failure(adapter):
    adapter._default_schedd.query.side_effect = Exception("connection refused")
    result = await adapter.ping_schedd("test-schedd")
    assert result is False
