...
async def metrics(repo: Repository = Depends(get_repository)):
    counts = await repo.count_requests_by_status()
    lines = []
    lines.append("# HELP condora_requests_total Number of requests by status")
    lines.append("# TYPE condora_requests_total gauge")
    for status_val, count in counts.items():
        lines.append(f'condora_requests_total{{status="{status_val}"}} {count}')
    return "\n".join(lines) + "\n"
...
async def htcondor_overview(
    ...
    from condora.core.monitoring_collector import collect_htcondor_overview
