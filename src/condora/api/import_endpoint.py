"""POST /api/v1/import — import a request from ReqMgr2 into Condora."""
...
from condora.core.dag_planner import DAGPlanner, PileupResolutionError
...
from condora.models.enums import RequestStatus
...
async def import_request(
    ...
    if existing:
        ...
        if existing.status not in reimportable:
            raise HTTPException(
                status_code=409,
                detail=f"Request already exists in Condora (status: {existing.status}). "
    ...
    if body.stageout_mode == "test":
        if not settings.test_lfn_user:
            raise HTTPException(
                status_code=400,
                detail="CONDORA_TEST_LFN_USER must be set for test stageout mode",
            )
