...
document.addEventListener('alpine:init', () => {
    Alpine.data('requestDetail', (requestName) => ({
        ...
        async fetchAll() {
            try {
                ...
                const [req, wf] = await Promise.all([
                    CONDORA_API.getRequest(this.name),
                    CONDORA_API.getWorkflowByRequest(this.name).catch(() => null),
                ]);
                this.request = req;
                this.workflow = wf;
                ...
                if (wf) {
                    const [dags, ods] = await Promise.all([
                        CONDORA_API.getWorkflowDags(wf.id).catch(() => []),
                        CONDORA_API.getWorkflowOutputDatasets(wf.id).catch(() => []),
                    ]);
            ...
            Promise.all([
                CONDORA_API.getRequestErrors(this.name).catch(() => null),
                CONDORA_API.getSitePerformance(this.name).catch(() => null),
            ]).then(([errs, sitePerf]) => {
                this.errors = errs;
        ...
        async savePriority() {
            ...
            try {
                await CONDORA_API.updatePriorityProfile(this.name, {
        ...
        async doStop() {
            this.actionLoading = true;
            try {
                const result = await CONDORA_API.stopRequest(this.name, this.stopReason);
                this.toast('success', result.message);
        ...
        async doFail() {
            this.actionLoading = true;
            try {
                const result = await CONDORA_API.failRequest(this.name);
                this.toast('success', result.message);
        ...
        async doClone() {
            this.actionLoading = true;
            try {
                const result = await CONDORA_API.cloneRequest(this.name);
        ...
        async doDelete() {
            this.actionLoading = true;
            try {
                const result = await CONDORA_API.deleteRequest(this.name);
                this.toast('success', result.message);
