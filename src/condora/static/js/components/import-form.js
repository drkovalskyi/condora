...
document.addEventListener('alpine:init', () => {
    Alpine.data('importForm', () => ({
        ...
        async loadPreview() {
            ...
            try {
                this.preview = await CONDORA_API.previewRequest(name);
        ...
        async submit() {
            ...
            try {
                const result = await CONDORA_API.importRequest(this._buildBody());
        ...
        async doReplace() {
            ...
            try {
                const result = await CONDORA_API.importRequest(this._buildBody(true));
                window.dispatchEvent(new CustomEvent('condora:toast', {
                    detail: { type: 'success', message: result.message }
                }));
                window.location.href = '/ui/requests/' + encodeURIComponent(result.request_name);
