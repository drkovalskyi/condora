...
from condora.adapters.mock import MockDBSAdapter as _MockDBS, MockRucioAdapter as _MockRucio
...
from condora.core.lifecycle_manager import complete_round
from condora.core.output_lfn import derive_merged_lfn_bases, determine_merged_lfn_base
...
from condora.models.enums import DAGStatus, RequestStatus
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        prog="condora",
        description="Condora CLI — import and run real workflows",
    )
    ...
    imp.add_argument("--submit-dir", default="/mnt/shared/tmp/condora", help="DAG file output directory")
...
def _build_adapters(settings: Settings, ssl_ctx: ssl.SSLContext):
    ...
    from condora.adapters.dbs import DBSClient
    from condora.adapters.reqmgr2 import ReqMgr2Client
    from condora.adapters.rucio import RucioClient
...
async def run_import(args: argparse.Namespace) -> None:
    ...
    print(f"=== Condora CLI: importing {request_name} ===")
    ...
    try:
        async with session_factory() as session:
            ...
            if args.processing_version is not None:
                from condora.api.import_endpoint import _apply_processing_version
            ...
            if args.merged_lfn_base:
                ...
                if not settings.test_lfn_user:
                    print("ERROR: CONDORA_TEST_LFN_USER must be set for test stageout mode")
                    sys.exit(1)
            ...
            if reqdata.get("RequestType") == "StepChain" and reqdata.get("StepChain"):
                ...
                from condora.core.stepchain import parse_stepchain
            ...
            # (CONDORA_SPOOL_MOUNT, CONDORA_REMOTE_SPOOL_PREFIX, CONDORA_REMOTE_SCHEDD).
            ...
            if dry_run:
                ...
                from condora.adapters.mock import MockCondorAdapter as _MockCondor
                ...
                while True:  # inner loop: poll + rescue within a round
                    ...
                    if result.action == "rescue":
                        ...
                        if result.problem_sites:
                            from condora.core.error_handler import ErrorHandler as EH
