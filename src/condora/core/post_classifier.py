...
- generate_collector_script() — returns self-contained Python script string
  that the DAG Planner writes to {submit_dir}/condora_post_collect.py
...
adapted for Condora's two-level error handling model (docs/error_handling.md).
"""
...
def generate_collector_script() -> str:
    """Return self-contained Python script (condora_post_collect.py).
    ...
    Called as: python3 condora_post_collect.py <node> <exit_code> <retry> <max_retries>
    """
    return r'''#!/usr/bin/env python3
"""condora_post_collect.py — POST script data collector (self-contained).

Called by post_script.sh after each job attempt. Parses FJR, HTCondor logs,
classifies errors, and writes {node_name}.post.json.

Usage: python3 condora_post_collect.py <node_name> <exit_code> <retry_num> <max_retries>
"""
...
def parse_condor_log(log_path):
    ...
    # with PeriodicRemoveReason on the next line.  Look for our "Condora:" prefix.
    m = re.search(r'009 \(.*?\).*?Job was aborted', content)
    if m:
        # Search for Condora reason in lines following the abort event
        after_abort = content[m.start():]
        rm = re.search(r'Condora: (zombie detected.*|hard wall time limit.*)', after_abort)
        if rm:
            info["periodic_remove_reason"] = "Condora: " + rm.group(1).strip()
...
def main():
    if len(sys.argv) < 5:
        print("Usage: condora_post_collect.py <node_name> <exit_code> <retry_num> <max_retries> [group_dir]",
              file=sys.stderr)
        sys.exit(1)
