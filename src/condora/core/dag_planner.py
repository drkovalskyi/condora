...
from condora.adapters.base import CondorAdapter, DBSAdapter, RucioAdapter
...
PILEUP_CACHE_FILE = "/mnt/shared/tmp/condora/pileup_cache.json"
...
class DAGPlanner:
    ...
    async def submit_pilot(self, workflow) -> None:
        ...
        from condora.core.pilot_runner import write_pilot_script
        ...
        pilot_script_path = os.path.join(submit_dir, "condora_pilot.py")
    ...
    def _parse_pilot_report(self, path: str) -> PilotMetrics:
        ...
        from condora.core.pilot_runner import parse_pilot_report
    ...
    async def plan_production_dag(
        ...
        *, adaptive: bool = False,
        ...
        from condora.core.schedd_selector import select_schedd
...
def _generate_dag_files(
    ...
    if spool_mode:
        outer_lines = [
            f"# Condora-generated DAG for workflow {workflow_id}",
    ...
    for mg in merge_groups:
        ...
        if spool_mode:
            ...
            for script in ("elect_site.sh", "pin_site.sh", "post_script.sh",
                            "condora_post_collect.py", "landing_validate.sh"):
                link = mg_dir / script
                if not link.exists():
...
def _generate_group_dag(
    ...
    if spool_mode:
        ...
        fp = f"{mg_name}/"
        proc_exe = "condora_proc.sh"
        merge_exe = "condora_merge.py"
        cleanup_exe = "condora_cleanup.py"
    else:
        fp = ""
        proc_exe = "../condora_proc.sh"
        merge_exe = "../condora_merge.py"
    ...
    # Add condora_stageout.py to transfer files for grid mode
    stageout_utility_path = str(submit_dir / "condora_stageout.py")
    if stageout_mode == "grid" and os.path.isfile(stageout_utility_path):
        if spool_mode:
            proc_transfer_files.append("condora_stageout.py")
        else:
            proc_transfer_files.append("../condora_stageout.py")
    ...
    if not skip_site_pinning:
        ...
        landing_classads["CONDORA_QuickJob"] = "True"
        ...
        if stageout_mode == "grid" and output_info_path:
            ...
            if spool_mode:
                ...
                landing_transfer = [f"{fp}output_info.json", "condora_stageout.py"]
            else:
                landing_exe = "../landing_validate.sh"
                landing_transfer = ["output_info.json", "../condora_stageout.py"]
            _write_submit_file(
    ...
    if stageout_mode == "grid" and os.path.isfile(stageout_utility_path):
        if spool_mode:
            merge_transfer.append("condora_stageout.py")
        else:
            merge_transfer.append("../condora_stageout.py")
    ...
    if stageout_mode == "grid" and os.path.isfile(stageout_utility_path):
        if spool_mode:
            cleanup_transfer.append("condora_stageout.py")
        else:
            cleanup_transfer.append("../condora_stageout.py")
    _write_submit_file(
...
def _write_submit_file(
    ...
    # can report step progress (CONDORA_CurrentStep, CONDORA_Phase, etc.) back
...
def _generate_pilot_submit(
    ...
    transfer_inputs = ["condora_pilot.py"]
    if sandbox_path and os.path.isfile(sandbox_path):
        transfer_inputs.append(sandbox_path)
    lines = [
        "# Condora pilot job",
        "universe = vanilla",
        "executable = condora_pilot.py",
        f"arguments = {args}",
        "output = pilot.out",
        "error = pilot.err",
        "log = pilot.log",
...
CLASSIFICATION=$(python3 "$SCRIPT_DIR/condora_post_collect.py" \
...
for pj in "${GROUP_DIR}"/proc_*.post.json; do
    [ -f "$pj" ] || continue
    # Only count actual failures (exit_code != 0), not successful completions
    pj_rc=$(grep -o '"exit_code": *[0-9-]*' "$pj" 2>/dev/null | head -1 | grep -o '[0-9-]*$')
    [ "${pj_rc:-0}" != "0" ] && any_fail_count=$((any_fail_count + 1))
done
half=$((PROC_COUNT / 2))
...
def _write_post_collector(path: str) -> None:
    from condora.core.post_classifier import generate_collector_script
...
START_TIME=$(date +%s)
echo "=== Condora Processing Wrapper ==="
...
def stage_file(local_path, lfn):
    ...
    if stageout_mode == 'grid':
        pfn, cmd = condora_stageout.resolve_write_pfn(lfn, siteconfig)
        pfn_dir = os.path.dirname(pfn)
        condora_stageout.mkdir_p(pfn_dir, cmd)
        condora_stageout.upload_file(local_path, pfn, cmd)
    else:
...
inject_pset_parallel() {
    ...
    python3 -c "
pset_path = '$pset'
lines = ['', '# --- Condora parallel instance PSet injection ---']
...
SETUPEOF
    ...
    if [[ "$use_tmpfs" == "true" ]]; then
        TMPFS_BASE="/dev/shm/condora_step0_$$"
...
import os
pset = '$pset_file'
lines = ['', '# --- Condora pipeline PSet injection ---']
lines.append('import FWCore.ParameterSet.Config as cms')
...
if _pu_found:
    ...
    if os.path.isfile(_manifest_path):
        ...
        if _ds:
            ...
            lines.append('            print(\"Condora: Overrode \" + _mixer + \".input.fileNames with \" + str(len(_pu_pfns)) + \" pileup files \" + _how)')
else:
    ...
    lines.append('            print(\"Condora: Prefixed \" + str(len(_old)) + \" existing pileup LFNs in \" + _mixer + \" with AAA global redirector\")')
...
run_cmssw_mode() {
    ...
    # - tmpfs_present: 1 if /dev/shm/condora_step0_$$ exists, else 0
    ...
    (
        ...
        while true; do
            ...
            tmpfs_flag=0
            [[ -d /dev/shm/condora_step0_${_wrapper_pid} ]] && tmpfs_flag=1
    ...
    for step_idx in $(seq 0 $((NUM_STEPS - 1))); do
        ...
        if [[ $step_idx -eq 0 && "$N_PARALLEL" -gt 1 ]]; then
            ...
            condor_chirp set_job_attr CONDORA_CurrentStep "1" 2>/dev/null || true
            condor_chirp set_job_attr CONDORA_StepName "\"$STEP_NAME\"" 2>/dev/null || true
            condor_chirp set_job_attr CONDORA_NumSteps "$NUM_STEPS" 2>/dev/null || true
            condor_chirp set_job_attr CONDORA_Phase "\"cmsRun\"" 2>/dev/null || true
            STEP_START=$(date +%s)
            ...
            condor_chirp set_job_attr CONDORA_StepDone "1" 2>/dev/null || true
            condor_chirp set_job_attr CONDORA_StepWallSec "$STEP_WALL" 2>/dev/null || true
            continue
        fi
...
import os
pset = '$PSET_PATH'
lines = ['', '# --- Condora runtime PSet injection ---']
lines.append('import FWCore.ParameterSet.Config as cms')
...
" 2>/dev/null)
        ...
        if [[ $step_idx -eq 0 && "$SPLIT_TMPFS" == "true" && "$N_PARALLEL" -le 1 ]]; then
            STEP0_TMPFS="/dev/shm/condora_step0_$$"
            mkdir -p "$STEP0_TMPFS"
        ...
        condor_chirp set_job_attr CONDORA_StepDone "$step_num" 2>/dev/null || true
        condor_chirp set_job_attr CONDORA_StepWallSec "$STEP_WALL" 2>/dev/null || true
...
" 2>/dev/null || true
    ...
    # Stage out to unmerged site storage
    condor_chirp set_job_attr CONDORA_Phase "\"stageout\"" 2>/dev/null || true
    condor_chirp set_job_attr CONDORA_StageoutStart "$(date +%s)" 2>/dev/null || true
    stage_out_to_unmerged
}
...
"""condora_merge.py — Condora merge job.
...
"""condora_cleanup.py — Condora cleanup job.
...
def _write_stageout_utility(path: str) -> None:
    """Generate condora_stageout.py — self-contained stageout utility for worker nodes.
    ...
    """
    _write_file(path, '''#!/usr/bin/env python3
"""condora_stageout.py — Self-contained stageout utility for Condora worker nodes.
...
No Condora imports — stdlib only. Transferred to worker nodes by DAG planner.
"""
import json
import os
import re
...
if __name__ == "__main__":
    if len(sys.argv) >= 2 and sys.argv[1] == "--validate":
        ...
        pfn_dir = os.path.dirname(pfn)
        mkdir_p(pfn_dir, cmd)
        test_local = "/tmp/.condora_validate_test"
...
TEST_LFN="${UNMERGED_LFN_BASE}/.condora_stageout_test_$$"
RESULT=$(python3 condora_stageout.py --validate "$TEST_LFN" "$SITECONFIG" 2>&1) || {
