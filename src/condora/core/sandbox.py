"""Sandbox builder: creates Condora sandbox tarballs with manifest + PSet configs."""
...
def create_sandbox(
    ...
    """Create a Condora sandbox tar.gz with manifest.json + PSet configs.
        ...
        if mode == "cmssw":
            if not ssl_context or not configcache_url:
                raise RuntimeError(
                    ...
                    "Set CONDORA_CERT_FILE/CONDORA_KEY_FILE environment variables or use "
...
def create_sandbox_from_spec(
    ...
    Returns path to created tar.gz.
    """
    from condora.core.stepchain import StepChainSpec, to_manifest
...
def _build_manifest(request_data: dict[str, Any], mode: str) -> dict[str, Any]:
    ...
    if request_data.get("RequestType") == "StepChain" and request_data.get("StepChain"):
        from condora.core.stepchain import parse_stepchain, to_manifest

        spec = parse_stepchain(request_data)
        return to_manifest(spec)

    # Legacy single-step CMSSW mode
    from condora.core.stepchain import _normalize_arch
