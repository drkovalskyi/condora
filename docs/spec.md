...
**In-job CPU watchdog classification**: When the in-job watchdog kills a stuck proc (prints `WATCHDOG: CPU stalled` to stderr), the POST collector reads the stderr tail and classifies it as `infrastructure` (retryable with cooloff). DAGMan RETRY sends the job to a different machine. If the issue is machine-specific (dead glidein), the retry succeeds. If it's site-wide, multiple WU procs stall, the failure ratio triggers WU-level or DAG-level error handling.
